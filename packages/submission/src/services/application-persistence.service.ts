...
export async function persistApplicationToSupabase(params: {
  ...
  const { data: appRow, error: appError } = await supabaseAdmin
    ...
    .insert({
      ...
      email: payload.email,
      phone: payload.phone,
      postal_address: payload.postalAddress?.trim() || null,
  ...
  for (const entity of core.entities) {
    ...
    const { data: entRow, error: entError } = await supabaseAdmin
      ...
      .insert({
        ...
        bank_account_count: entity.bankAccountCount,
        foreign_bank_account_count: entity.foreignBankAccountCount,
        loan_count: entity.loanCount,
        cryptocurrency_count: entity.cryptocurrencyCount,
        has_primary_bank_account: entity.hasPrimaryBankAccount === true,
        primary_bank_name: entity.hasPrimaryBankAccount ? entity.primaryBankName?.trim() || null : null,
        primary_bank_account_name: entity.hasPrimaryBankAccount ? entity.primaryBankAccountName?.trim() || null : null,
        primary_bank_bsb: entity.hasPrimaryBankAccount ? entity.primaryBankBsb?.replace(/\s/g, "") || null : null,
        primary_bank_account_number: entity.hasPrimaryBankAccount
          ? entity.primaryBankAccountNumber?.replace(/\s/g, "") || null
          : null,
