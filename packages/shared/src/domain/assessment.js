import { COMPLEXITY_BANDS, COMPLEXITY_POINTS, ENTITY_BASE_FEES, JM_ONLY_SERVICES, ONBOARDING_FEES, OTHER_ADD_ONS, REPORTING_ADD_ONS, } from "../constants/pricing";
function calculateComplexityPoints(entity) {
    return (entity.listedInvestmentCount * COMPLEXITY_POINTS.listedInvestment +
...
function deriveGroupDiscount(pgEntityCount, annualSubtotal) {
    if (pgEntityCount <= 2)
        return 0;
    if (pgEntityCount === 3)
        return annualSubtotal * 0.05;
    if (pgEntityCount <= 5)
        return annualSubtotal * 0.1;
    return 0;
}
function assessEntity(entity) {
    const jmReasons = [];
    const reviewReasons = [];
    ...
    const complexityBand = COMPLEXITY_BANDS.find((band) => complexityPoints >= band.min && complexityPoints <= band.max);
    if (!complexityBand) {
    ...
    let routingOutcome = "pg_fit";
    let pricingStatus = "indicative";
    let annualFeeEstimate = null;
    let onboardingFeeEstimate = null;
    if (jmReasons.length > 0) {
        ...
        const baseFee = ENTITY_BASE_FEES[entity.entityType] ?? null;
        const reportingAddOns = entity.serviceCodes.reduce((sum, serviceCode) => {
            return sum + (REPORTING_ADD_ONS[serviceCode] ?? 0);
        }, 0);
        const serviceAddOns = entity.serviceCodes.reduce((sum, serviceCode) => {
            return sum + (OTHER_ADD_ONS[serviceCode] ?? 0);
        }, 0);
        if (baseFee === null || !complexityBand) {
            ...
            onboardingFeeEstimate =
                ONBOARDING_FEES[entity.portfolioStatus] ?? null;
...
export function assessApplication(input) {
    ...
    const requiresManualReview = entityAssessments.some((entity) => entity.routingOutcome === "manual_review");
    const annualSubtotal = pgEntities.reduce((sum, entity) => sum + (entity.annualFeeEstimate ?? 0), 0);
    const onboardingSubtotal = pgEntities.reduce((sum, entity) => sum + (entity.onboardingFeeEstimate ?? 0), 0);
    const groupDiscountAmount = pgEntities.length >= 6 ? 0 : deriveGroupDiscount(pgEntities.length, annualSubtotal);
    const overallOutcome = requiresManualReview
        ? "manual_review"
        : requiresJmFollowUp && pgEntities.length === 0
            ? "jm_fit"
            : "pg_fit";
    const indicativePricingAvailable = !requiresManualReview &&
        pgEntities.length > 0 &&
        pgEntities.length < 6 &&
        pgEntities.every((entity) => entity.pricingStatus === "indicative" &&
